...
static DEFINE_MUTEX(fault_mutex);
...
extern uint64_t first_tsc;

// flush virtual address
extern void *woomin_flush_va;
extern uint64_t flush_gfn;
extern uint64_t flush_gpa;
...
extern bool woomin2_tdx_sept_unzap_private_spte(struct kvm *kvm, gfn_t gfn,
                enum pg_level level);
extern void woomin_reload_direct(void);
...
static void handle_removed_pt(struct kvm *kvm, tdp_ptep_t pt, bool shared)
{
	...
	for (i = 0; i < SPTE_ENT_PER_PAGE; i++) {
		...
		handle_changed_spte(kvm, kvm_mmu_page_as_id(sp), gfn,
				    old_spte, FROZEN_SPTE, sp->role, shared);
	}
						 ...
						 u64 new_spte, int level)
{
	 ...
	 * freezing SPTE.
	 * - Freeze the SPTE. Set entry to FROZEN_SPTE.
					...
					struct tdp_iter *iter, u64 new_spte)
{
	...
	if (is_private_sptep(iter->sptep) && !is_frozen_spte(new_spte)) {
		int ret;
		if (is_shadow_present_pte(new_spte)) {
			 ...
			 */
			if (!try_cmpxchg64(sptep, &iter->old_spte, FROZEN_SPTE))
				return -EBUSY;
		}
	} else {
		/*
			    ...
			    u64 old_spte, u64 new_spte, gfn_t gfn, int level)
{
	...
	if (is_private_sptep(sptep) && !is_frozen_spte(new_spte) &&
	    is_shadow_present_pte(new_spte)) {
		lockdep_assert_held_write(&kvm->mmu_lock);
...
int kvm_tdp_mmu_map(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	...
	if(woomin_block == true)
	{
		tracker = false;
		mutex_lock(&fault_mutex);
		...
		for(int k = 0; k < woomin_fault_index ; k++)
		{
			...
			{
				woomin_tdx_sept_rezap_private_spte_fnptr(kvm, woomin_list[k], PG_LEVEL_4K);
			}

			if( (raw_gfn == woomin_list[k]))
			{
				tracker = true;
				fault_index = k;
			}
		...
		if( (tracker == true) )
		{
			...
			if(fault_count == 1)
			{
				...
				first_tsc = current_tsc - past_tsc;
//				unsigned int cpu_id = smp_processor_id();
				printk(KERN_ERR "[%s] GPN [0x%llx] time diff : %llu\n",woomin_str[fault_index], fault->addr >> 12, first_tsc); // current_tsc - past_tsc
																	       //printk(KERN_ERR "The cpu core [%d] is running\n", cpu_id);
																	       
				woomin_tdx_sept_unzap_private_spte_fnptr(kvm, fault_gfn, PG_LEVEL_4K);
//				woomin_tdx_reload_direct_fnptr();
				if (flush_tracker == true)
				{
					for(int temp_flush = 0; temp_flush < woomin_flush_index ; temp_flush++)
					{
						if(raw_gfn == woomin_flush_gfn[temp_flush])
						{
							woomin_tdx_flush_load_fnptr(temp_flush);
						}
					}
