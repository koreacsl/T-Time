...
// ===========================================================================================
extern uint64_t a,d, start, end;
extern bool woomin_block;
extern uint64_t fault_count;
extern uint64_t fault_gfn;
extern int woomin_fault_index;
extern int woomin_list[500];
extern bool woomin_blocked[500];
extern char woomin_str[500][30];
extern bool tracker;
extern uint64_t current_tsc;
extern uint64_t past_tsc;
extern uint64_t past_tsc_t;

// flush virtual address
extern void *woomin_flush_va[10];
extern struct page *woomin_flush_page[10];
extern uint64_t woomin_flush_gfn[10];
extern uint64_t woomin_flush_gpa[10];
extern volatile uint64_t flush_data;
extern bool flush_tracker;
extern int woomin_flush_index;


extern void *woomin_flush_direct_va;
extern struct page *woomin_flush_direct_page;

static DEFINE_MUTEX(woomin_mutex);
static DEFINE_MUTEX(woomin_fault_mutex);

u64 tdh_mem_range_unblock_woomin(hpa_t tdr, gpa_t gpa, int level,
                struct tdx_module_args *out);
u64 tdh_mem_range_block_woomin(hpa_t tdr, gpa_t gpa, int level,
                struct tdx_module_args *out);
int tdx_sept_unzap_private_spte(struct kvm *kvm, gfn_t gfn,
                enum pg_level level);
int tdx_sept_zap_private_spte(struct kvm *kvm, gfn_t gfn,
                enum pg_level level);
bool woomin_tdx_sept_zap_private_spte(struct kvm *kvm, gfn_t gfn,
                enum pg_level level);
bool woomin_tdx_sept_unzap_private_spte(struct kvm *kvm, gfn_t gfn,
                enum pg_level level);
bool woomin_tdx_sept_rezap_private_spte(struct kvm *kvm, gfn_t gfn,
                enum pg_level level);

bool woomin_list_remove(int index);
...
bool woomin_tdx_list_all_block(struct kvm *kvm);
bool woomin_tdx_list_all_unblock(struct kvm *kvm);

void tdx_track(struct kvm *kvm);
bool woomin_list_print(void);
int woomin_set_cpu_frequency(unsigned int cpu,unsigned int new_freq);
int woomin_array_tag(int index, const char *string);
int woomin_flush(struct kvm_vcpu *vcpu, gpa_t gpa);
void woomin_flush_load(int num);
bool woomin_flush_list_print(void);
bool woomin_flush_remove_all(void);
int woomin_flush_direct(struct kvm_vcpu *vcpu, gpa_t gpa);
int woomin_reload_direct(void);
// ===========================================================================================
...
//	kvm_mmu_set_mmio_spte_value(kvm, 0);
	...
	kvm->arch.has_private_mem = true;
        ...
        fault_count = 0;
        fault_gfn =0;
        woomin_fault_index = 0;

        woomin_tdx_sept_zap_private_spte_fnptr = woomin_tdx_sept_zap_private_spte;
        woomin_tdx_sept_unzap_private_spte_fnptr = woomin_tdx_sept_unzap_private_spte;
        woomin_tdx_sept_rezap_private_spte_fnptr = woomin_tdx_sept_rezap_private_spte;
	woomin_tdx_flush_load_fnptr = woomin_flush_load;
	woomin_tdx_reload_direct_fnptr = woomin_reload_direct;
...
// woomin test
	tdx->exit_reason.full = __seamcall_saved_ret_woomin(TDH_VP_ENTER, &args, &current_tsc, &past_tsc_t);
                ...
                struct tdx_module_args *out)
{
        ...
        //        pr_info("[woomin] [tdh_mem_range_block] tdr : 0x%llx, gpa : 0x%llx, level : %d\n", tdr, gpa, level);
        return tdx_seamcall_sept(TDH_MEM_RANGE_BLOCK, &in, out);
}

EXPORT_SYMBOL_GPL(tdh_mem_range_block_woomin);
                ...
                struct tdx_module_args *out)
{
        ...
        //        pr_info("[woomin] [tdh_mem_range_unblock] tdr : 0x%llx, gpa : 0x%llx, level : %d\n", tdr, gpa, level);

        return tdx_seamcall_sept(TDH_MEM_RANGE_UNBLOCK, &in, out);
}
EXPORT_SYMBOL_GPL(tdh_mem_range_unblock_woomin);

EXPORT_SYMBOL_GPL(tdx_inject_nmi);
...
EXPORT_SYMBOL_GPL(tdx_sept_zap_private_spte);
                ...
                enum pg_level level)
{
        ...
        gpa_t gpa = gfn_to_gpa(gfn);
        struct tdx_module_args out;
        for(int k = 0; k < woomin_fault_index ; k++)
        {
                if( (gfn == woomin_list[k]) )
                {
                        printk("[woomin_zap] already in list\n");
                        tdh_mem_range_block(kvm_tdx->tdr_pa, gpa, tdx_level, &out);
                        return false;
                }

        }
        woomin_list[woomin_fault_index] = gfn;
        woomin_blocked[woomin_fault_index] = true;

        woomin_fault_index += 1;
        u64 err;
        ...
        printk("[woomin][zap]guest frame number : 0x%llx, fault_gfn :  0x%llx\n", gpa, fault_gfn);
...
EXPORT_SYMBOL_GPL(woomin_tdx_sept_zap_private_spte);
                ...
                enum pg_level level)
{
        ...
        for(int k = 0; k < woomin_fault_index ; k++)
        {
                if( (gfn == woomin_list[k]) )
                {
                        woomin_blocked[k] = true;
                }

        }

        //      printk("[woomin][rezap]guest frame number : 0x%llx, fault_gfn : %llx\n", gpa, fault_gfn);

        mutex_unlock(&woomin_mutex);


        return true;

}
EXPORT_SYMBOL_GPL(woomin_tdx_sept_rezap_private_spte);
                ...
                enum pg_level level)
{
        ...
        //      printk("[woomin][unzap]guest frame number : 0x%llx, fault_gfn : 0x%llx\n", gpa, fault_gfn);

        return true;

}

EXPORT_SYMBOL(woomin_tdx_sept_unzap_private_spte);

bool woomin_list_print(void)
{
        if(woomin_fault_index == 0)
        {
                printk("Nothing in woomin_list\n");
        ...
        else
        {
                ...
                for(int k = 0; k< woomin_fault_index ; k++)
                {
                        printk("[woomin_list_print] woomin_list[%d][%s] = 0x%x (blocked = %d)\n", k,woomin_str[k],woomin_list[k], woomin_blocked[k]);
...
EXPORT_SYMBOL(woomin_list_print);

bool woomin_list_remove(int index)
{
        if (index < 0 || index >= woomin_fault_index) {
                printk("[woomin_list_remove] failed.. invalid index range\n");
                return false; // 유효하지 않은 인덱스면 false 반환
        }

        if( (index == (woomin_fault_index - 1)) )
        {
                woomin_fault_index--;
                woomin_list[woomin_fault_index] = 0;
                memset(woomin_str[index], '\0', 30);
                woomin_blocked[woomin_fault_index] = false;
                woomin_fault_index++;
        }


        mutex_lock(&woomin_mutex);
        // index부터 woomin_fault_index - 1까지 값을 한 칸씩 앞으로 이동
        for (int i = index; i < woomin_fault_index - 1; i++) {
                woomin_list[i] = woomin_list[i + 1];
                woomin_blocked[i] = woomin_blocked[i + 1];
                strncpy(woomin_str[i], woomin_str[i + 1], 29);
                woomin_str[i][29] = '\0';  // 문자열 끝에 null-terminator 추가

        }
        mutex_unlock(&woomin_mutex);

        // 마지막 값을 제거한 효과를 위해  woomin_fault_index 감소
        woomin_fault_index--;

        printk("[woomin_list_remove] list remove in index %d\n", index);

        return true; // 성공적으로 제거하면 true 반환

}

EXPORT_SYMBOL(woomin_list_remove);

bool woomin_list_remove_all(struct kvm *kvm)
{
        mutex_lock(&woomin_fault_mutex);

        for(int k = 0; k< woomin_fault_index ; k++)
        {
                woomin_tdx_sept_unzap_private_spte_fnptr(kvm, woomin_list[k], PG_LEVEL_4K);
                memset(woomin_str[k], '\0', 30);
                woomin_list[k] = -1;
        }


        woomin_block = false;
        mutex_unlock(&woomin_fault_mutex);


        woomin_fault_index = 0;
        printk("[woomin_list_remove_all] remove all list \n");

        return true;
}

EXPORT_SYMBOL(woomin_list_remove_all);
...
EXPORT_SYMBOL(woomin_split_all_pages);

bool woomin_tdx_list_all_block(struct kvm *kvm)
{
        for(int k = 0; k < woomin_fault_index ; k++)
        {
                if(woomin_blocked[k] == false)
                {
                        woomin_tdx_sept_rezap_private_spte(kvm, woomin_list[k], PG_LEVEL_4K);
                }

        }

        printk("[woomin][tdx_list_all_block] all blocked!\n");
        woomin_block = true;
...
EXPORT_SYMBOL(woomin_tdx_list_all_block);

bool woomin_tdx_list_all_unblock(struct kvm *kvm)
{
        for(int k = 0; k < woomin_fault_index ; k++)
        {
                if(woomin_blocked[k] == true)
                {
                        woomin_tdx_sept_unzap_private_spte(kvm, woomin_list[k], PG_LEVEL_4K);
                }

        }

        printk("[woomin][tdx_list_all_unblock] all unblocked!\n");
        woomin_block = false;
...
EXPORT_SYMBOL(woomin_tdx_list_all_unblock);
...
int woomin_set_cpu_frequency(unsigned int cpu,unsigned int new_freq) {
        ...
        if(!policy) {
                printk("%s : cpufreq_cpu_get failed\n", __FUNCTION__);
                cpufreq_cpu_put(policy);
                return -1;
        }

        actual_freq = cpufreq_driver_fast_switch(policy, new_freq);
        if( 0 == actual_freq ) {
                printk("%s : cpufreq_driver_fast_switch failed\n", __FUNCTION__);
                cpufreq_cpu_put(policy);
                return -1;
        }       if(actual_freq != new_freq ) {
                printk("%s: frequency was set to %u instead of %u\n", __FUNCTION__, actual_freq, new_freq);
                cpufreq_cpu_put(policy);
                return -1;
        }
        cpufreq_cpu_put(policy);
        printk("%s : set frequency on core %d to %u\n", __FUNCTION__, cpu, actual_freq);
        //debugging: busy wait
        start = ktime_get();
        end = ktime_add_ns(start, 100000000);
        while (ktime_before(ktime_get(), end))
                cpu_relax();
        return 0;

}
EXPORT_SYMBOL(woomin_set_cpu_frequency);

int woomin_sept_gpa_to_hpa(struct kvm_vcpu *vcpu, uint64_t gpa, int target_level,uint64_t* out_hpa) {
        int err;
        int tdx_level = pg_level_to_tdx_sept_level(target_level);
        struct kvm_tdx *kvm_tdx = to_kvm_tdx(vcpu->kvm);
        struct tdx_module_args out;
        union tdx_sept_entry entry;
        uint64_t offset;

        //store offset and align gpa to page before querying the translation
        offset = gpa & (PAGE_SIZE - 1);
        gpa = (gpa >> PAGE_SHIFT) << PAGE_SHIFT;
        err = tdh_mem_sept_rd(kvm_tdx->tdr_pa, gpa, tdx_level, &out);
        if (err) {
                printk("%s: tdh_mem_sept_rd failed\n", __FUNCTION__);
                pr_tdx_error(TDH_MEM_SEPT_RD, err, &out);
                return -1;
        }
        entry.raw = out.rcx;
        //Table 4.12 says this is HPA[51:12], i.e. frame number
        *out_hpa = (entry.pfn << 12) | offset;
        return 0;
}
EXPORT_SYMBOL(woomin_sept_gpa_to_hpa);
...
int woomin_array_tag(int index, const char *str)
{
        ...
        //woomin_str[index] = str;
        strncpy(woomin_str[index], str, 29);
        woomin_str[index][29] = '\0';  // 문자열 끝에 null-terminator 추가
        printk("woomin_str[index] = %s", woomin_str[index]);
        for(int k = 0; k< woomin_fault_index ; k++)
        {
                printk("[woomin_list_print] woomin_list[%d][%s] = 0x%x (blocked = %d)\n", k,woomin_str[k],woomin_list[k], woomin_blocked[k]);
...
EXPORT_SYMBOL(woomin_array_tag);

int woomin_flush_direct(struct kvm_vcpu *vcpu, gpa_t gpa)
{

        hpa_t temp_direct_hpa;
        hpa_t hpa_with_hkid;
	uint64_t flush_gpa_direct;
	uint64_t flush_gfn_direct;

        kunmap(woomin_flush_direct_page);
//	struct page *woomin_flush_direct_page;

        uint64_t direct_start, direct_end, direct_diff;
//        unsigned long* woomin_flush_direct_va;

        flush_gpa_direct = gpa;
        flush_gfn_direct = gpa >> PAGE_SHIFT;

        woomin_sept_gpa_to_hpa(vcpu, gpa, PG_LEVEL_4K, &temp_direct_hpa);
        hpa_with_hkid = set_hkid_to_hpa(temp_direct_hpa, tdx_global_keyid);

        woomin_flush_direct_page = pfn_to_page(temp_direct_hpa >> PAGE_SHIFT);
        woomin_flush_direct_va = kmap(woomin_flush_direct_page);
        woomin_flush_direct_va = (void *)((unsigned long)woomin_flush_direct_va + (gpa & (PAGE_SIZE - 1)));


        printk("[woomin_flush] Set GPA 0x%llx flush address", gpa);

	/////////////////////////////////
        direct_start = rdtsc_ordered();
        __asm__("lfence");

        volatile uint64_t data = *(uint64_t *)woomin_flush_direct_va;

        direct_end = rdtsc_ordered();
        direct_diff = direct_end - direct_start;

	/////////////////////////////////
        printk("[woomin_flush] GPA 0x%llx to HVA 0x%p time diff = %llu", gpa, woomin_flush_direct_va, direct_diff);
        printk("Data at VA: 0x%llx | value : %llx\n", *(uint64_t *)woomin_flush_direct_va, data);
        

        return 1;
}
EXPORT_SYMBOL(woomin_flush_direct);
...
int woomin_reload_direct(void)
{
	...
	direct_start = rdtsc_ordered();
        flush_data = *(volatile uint64_t *)woomin_flush_direct_va;
	direct_end = rdtsc_ordered();
	direct_diff = direct_end - direct_start;
	printk("[woomin_reload_direct] time diff = %llu", direct_diff);
	return 1;
}

EXPORT_SYMBOL(woomin_reload_direct);

int woomin_flush(struct kvm_vcpu *vcpu, gpa_t gpa)
{
	void *woomin_flush_temp_va;
	uint64_t flush_gpa;
	uint64_t flush_gfn;
	
	struct page *woomin_flush_temp_page;
	...
	flush_gpa = gpa;
	flush_tracker = true;	
	flush_gfn = gpa >> PAGE_SHIFT;	

	/////
	for(int k = 0; k < woomin_flush_index ; k++)
        {
                if( (flush_gpa == woomin_flush_gpa[k]) )
                {
                        printk("[woomin_flush] already in flush list\n");
                        return 0;
                }

        }

        woomin_flush_gfn[woomin_flush_index] = flush_gfn;
	woomin_flush_gpa[woomin_flush_index] = flush_gpa;
	////////
	
	woomin_sept_gpa_to_hpa(vcpu, gpa, PG_LEVEL_4K, &temp_hpa);
	hpa_with_hkid = set_hkid_to_hpa(temp_hpa, tdx_global_keyid);

	
	woomin_flush_temp_page = pfn_to_page(temp_hpa >> PAGE_SHIFT);
	woomin_flush_temp_va = kmap(woomin_flush_temp_page);
	woomin_flush_temp_va = (void *)((unsigned long)woomin_flush_temp_va + (gpa & (PAGE_SIZE - 1)));

	woomin_flush_page[woomin_flush_index] = woomin_flush_temp_page;
        woomin_flush_va[woomin_flush_index] = woomin_flush_temp_va;	

	printk("[woomin_flush] Set GPA 0x%llx flush address", gpa);

        woomin_flush_index += 1;

	return 1;
}

EXPORT_SYMBOL(woomin_flush);

void woomin_flush_load(int num) // Number how many? testing...
{

	flush_data = *(volatile uint64_t *)woomin_flush_va[num];

	__asm__ volatile("" ::: "memory");
	__asm__ volatile("mfence");
}

EXPORT_SYMBOL(woomin_flush_load);

bool woomin_flush_list_print(void)
{
        if(woomin_flush_index == 0)
        {
                printk("Nothing in woomin_flush_list\n");
        ...
        else
        {
                ...
                for(int k = 0; k< woomin_flush_index ; k++)
                {
                        printk("[woomin_flush_list] woomin_flush list[%d] = 0x%llx (VA : 0x%llx)\n", k, woomin_flush_gpa[k], (uint64_t)woomin_flush_va[k]);
...
EXPORT_SYMBOL(woomin_flush_list_print);
...
bool woomin_flush_remove_all(void)
{
        ...
        for(int k = 0; k < woomin_flush_index ; k++)
        {
		woomin_flush_gfn[k] = 0;
		woomin_flush_gpa[k] = 0;
		woomin_flush_va[k] = 0;
		kunmap(woomin_flush_page[k]);
        ...
        woomin_flush_index = 0;
        printk("[woomin_flush_remove_all] remove all list \n");

        return true;
}
EXPORT_SYMBOL(woomin_flush_remove_all);
...
EXPORT_SYMBOL_GPL(tdx_sept_unzap_private_spte);
...
EXPORT_SYMBOL_GPL(tdx_flush_tlb);
