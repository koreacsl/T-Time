...
MODULE_DESCRIPTION("TDX Print Kernel Module");
...
static int __init tdx_print_init(void)
{
    struct task_struct *task;
    ...
    pr_info("Running woomin_list_print and woomin_flush_list_print...\n");
    woomin_list_print();
    woomin_flush_list_print();
...
static void __exit tdx_print_exit(void)
{
    pr_info("Exiting TDX Print Module\n");
}

module_init(tdx_print_init);
module_exit(tdx_print_exit);
