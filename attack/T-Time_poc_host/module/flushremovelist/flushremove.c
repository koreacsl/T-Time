...
MODULE_DESCRIPTION("TDX Flush Remove Kernel Module");
...
static int __init tdx_flushremove_init(void)
{
    struct task_struct *task;
    ...
    pr_info("Running woomin_flush_remove_all...\n");
    woomin_flush_remove_all();
...
static void __exit tdx_flushremove_exit(void)
{
    pr_info("Exiting TDX Flush Remove Module\n");
}

module_init(tdx_flushremove_init);
module_exit(tdx_flushremove_exit);
