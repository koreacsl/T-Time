...
enum CodeLoactionName {
    CLN_target,
    CLN_start,
    //DOES NOT REFERECE VALID ARRAY ENTRY!!!
    CodeLocationLength, 
};
...
// For libasm_library.so	
    const uint64_t IDA_BASE = 0x1000;  // Base virtual address used by IDA when analyzing the binary
...
int get_target_base_vaddr(uint64_t* out_target_vaddr){
  int pid = -1;
    ...
    bool has_target_name = ends_with_suffix("libasm_library.so",maps_tmp->pathname);
