...
enum CodeLoactionName {
    CLN_mpi_select,
    CLN_mpi_core_montmul,
    CLN_mbedtls_mpi_exp_mod,
    CodeLocationLength,
};
...
char* code_location_name_to_string(enum CodeLoactionName c) {
    switch (c)
    {
     case CLN_mpi_select:
        return "mpi_select";
        break;
    case CLN_mpi_core_montmul:
        return "mpi_core_montmul";
        break;
        
    case CLN_mbedtls_mpi_exp_mod:
...
// For mbedtls	
    const uint64_t IDA_BASE = 0x18000;  // Base virtual address used by IDA when analyzing the binary
...
int get_target_base_vaddr(uint64_t* out_target_vaddr){
  int pid = -1;
    ...
    bool has_target_name = ends_with_suffix("libmbedcrypto.so.15",maps_tmp->pathname);
